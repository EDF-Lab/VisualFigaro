...
public class GWidgetCheckboxGridForm extends GWidget {
	...
	@Deprecated
	public GWidgetCheckboxGridForm() {
		super();
	...
	public GWidgetCheckboxGridForm(GObject p, GObjectInformation info, Vector<Vector<String>> args, Vector<WidgetClasses> wc, Vector<Vector<Object>> objectsArgs) {
		super(p, info);
		...
		//The counter used to augment the grid property given that some "line" of the grid represent in reality 3 lines 
		...
		for(int i=0; i<numberOfElements; ++i) {
			...
			if(!wc.get(i).equals(WidgetClasses.TEXTFIELD)){
			  add(new JLabel("All "), constraints);
			  ++constraints.gridx;
			  }
	...
	public boolean loadXML(Vector<Element> elements, boolean deeplyRooted) {
		...
		int widgetSize = widgets.size();
		
		if(elementsSize != widgetSize)
			return false;
		...
		for(int i=0; i<elementsSize; ++i) {
			...
			//widgets.get(i).loadXML(GXMLElementFactory.refactorElements(bufferVector, key), false);
...
System.err.println("xmlTags.size() : "+xmlTags.size());
		...
		for(int i=0; i<numberOfWidgets; ++i) {
			...
			bufferElem = new Element(information.getLanguage().getBDCTranslation("TOUS"));
